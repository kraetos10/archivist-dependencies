...
public extension VLCVideoPlayer {
    ...
    struct Statistics {
        ...
        public init(stats: VLCMedia.Stats) {
            ...
            // `sentPackets` / `sentBytes` / `sendBitrate` were dropped from
            // `VLCMedia.Stats` in our VLCKit build — they were streaming-out
            // counters relevant to the VLC sender role, not the player. Zero
            // them out to keep the upstream PlaybackInformation API stable.
            sentPackets = 0
            sentBytes = 0
            sendBitrate = 0
        }
