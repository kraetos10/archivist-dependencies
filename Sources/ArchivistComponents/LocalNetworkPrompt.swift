...
///
public struct LocalNetworkPrompt {
    public static func triggerLocalNetworkPrivacyAlert() {
        ...
        let addresses = Self.addressesOfDiscardServiceOnBroadcastCapableInterfaces()
    ...
    private static func addressesOfDiscardServiceOnBroadcastCapableInterfaces() -> [Data] {
        ...
        return sequence(first: start, next: { $0.pointee.ifa_next })
            .compactMap { identifier -> Data? in
                guard
                    (identifier.pointee.ifa_flags & UInt32(bitPattern: IFF_BROADCAST)) != 0,
                    let saffa = identifier.pointee.ifa_addr
                else { return nil }
                var result = Data(UnsafeRawBufferPointer(start: saffa, count: Int(saffa.pointee.sa_len)))
