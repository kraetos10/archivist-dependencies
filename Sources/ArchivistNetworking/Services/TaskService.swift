...
public nonisolated protocol TaskServiceType: Sendable {
    func getTaskById(config: ServerConfig, taskId: String) async throws -> TaskResponse
    func sendTaskCommand(config: ServerConfig, taskId: String, command: String) async throws
    func getAllTaskResults(config: ServerConfig) async throws -> [String: TaskResult]
    func getTaskResult(config: ServerConfig, name: String) async throws -> TaskResult
    func startTask(config: ServerConfig, name: String) async throws -> TaskResponse
    ...
    func getSchedule(config: ServerConfig, name: String) async throws -> TaskScheduleResponse
    func updateSchedule(config: ServerConfig, name: String, schedule: TaskScheduleRequest) async throws
    func deleteSchedule(config: ServerConfig, name: String) async throws
}

public nonisolated struct TaskService: TaskServiceType {
    public init() {}

    public func getTaskById(config: ServerConfig, taskId: String) async throws -> TaskResponse {
        let request = NetworkAPIRequest<TaskResponse>(config: config, path: .taskById(id: taskId))
        return try await request.execute().data
    }

    public func sendTaskCommand(config: ServerConfig, taskId: String, command: String) async throws {
    ...
    public func getTaskResult(config: ServerConfig, name: String) async throws -> TaskResult {
        let request = NetworkAPIRequest<TaskResult>(config: config, path: .taskByNameSpecific(name: name))
        return try await request.execute().data
    }

    public func startTask(config: ServerConfig, name: String) async throws -> TaskResponse {
    ...
    public func getSchedule(config: ServerConfig, name: String) async throws -> TaskScheduleResponse {
        let request = NetworkAPIRequest<TaskScheduleResponse>(config: config, path: .taskScheduleSpecific(name: name))
        return try await request.execute().data
    }

    public func updateSchedule(config: ServerConfig, name: String, schedule: TaskScheduleRequest) async throws {
    ...
    public func deleteSchedule(config: ServerConfig, name: String) async throws {
        let request = NetworkAPIRequest<EmptyResponse>(
            config: config,
            path: .taskScheduleSpecific(name: name),
            method: .delete
